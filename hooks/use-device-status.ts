...
export function useDeviceStatus() {
  ...
  const deviceIdRef = useRef<string>("truck_1")

  useEffect(() => {
    let cancelled = false

    const init = async () => {
      // Discover the first available device from the backend
      try {
        const devRes = await fetch(`${API_URL}/api/devices`)
        const devJson = await devRes.json()
        if (devJson.success && devJson.devices.length > 0) {
          deviceIdRef.current = devJson.devices[0]
        }
      } catch {
        // Backend unreachable — use default deviceId and fall back to mock data below
      }

      if (cancelled) return

      // Fetch the latest snapshot for the device
      try {
        const latestRes = await fetch(`${API_URL}/api/data/${deviceIdRef.current}/latest`)
        const latestJson = await latestRes.json()
        if (!cancelled && latestJson.success && Object.keys(latestJson.latest).length > 0) {
          setDeviceStatus(latestJson.latest as DeviceStatus)
          return
        }
      } catch {
    ...
    socket.on(
      "deviceData",
      (payload: { deviceId: string; type: string; timestamp: string; data: Record<string, unknown> }) => {
